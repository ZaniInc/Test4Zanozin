...
let WomanNft
 ...
 it("set family field nft" , async function () {
    ...
    let familyNftt = await nftfamily.createFamilyNFT(mannft.address , womannft.address , 0,0,owner.address,22);
   ...
   it("set kid nft" , async function () {
    ...
    let kid = await kidnft.generateNftKid(4,"Nikita",nftfamily.address,mannft.address,womannft.address,0,owner.address,28);
    await kid.wait();
    // expect(await TokenForPayy.connect(acc2).balanceOf(acc2.address)).to.eq(10000);
